...
class inventoryController extends Controller{
   ...
   */
   public function inventory($productCode){
      ...
      return view('app.ecommerce.products.inventory', compact('inventories','productCode','product','outlets','mainBranch','mainBranch'));
   }
   ...
   */
   public function delete_inventory($productCode,$branchCode){
      $inventory = product_inventory::where('product_code',$productCode)
                                 ->where('branch_code',$branchCode)
                                 ->where('business_code',Auth::user()->business_code)
                                 ->first();

      if($inventory->current_stock == "" || $inventory->current_stock == 0 ){
         product_inventory::where('product_code',$productCode)->where('branch_code',$branchCode)->where('business_code',Auth::user()->business_code)->delete();
         product_price::where('product_code',$productCode)->where('branch_code',$branchCode)->where('business_code',Auth::user()->business_code)->delete();
