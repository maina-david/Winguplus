...
class incomesummaryController extends Controller
{
   ...
   public function report(Request $request,$id){
      ...
      $expense = expense::where('businessID',Auth::user()->businessID)->where('propertyID',$id)->whereBetween('date',[$request->from,$request->to])->sum('amount');
      ...
      return view('app.property.property.reports.incomesummary', compact('income','expense','business','incomeCategories','from','to','unCategorisedInvoicesSum','unCategorisedInvoicesCount','expenseCategory','property','originalIncomes','propertyID'));
   }
   ...
   public function generate($id,$to,$from){
      ...
      $pdf = PDF::loadView('templates/'.Wingu::template($business->templateID)->template_name.'/reports/property/incomesummery', compact('income','expense','business','incomeCategories','from','to','unCategorisedInvoicesSum','unCategorisedInvoicesCount','expenseCategory','originalIncomes','property'));
