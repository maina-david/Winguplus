...
class TaskSection extends Component
{
   ...
   public $sectionCode;
   public $taskDetails;
   public $group_title,$groupCode,$taskCode,$color,$group_section;
   ...
   //section
   public $editSectionModal;
   public $SectionEditCode;
   ...
   public function render()
   {
      ...
      $code = $this->jobCode;
      $groups = group::where('business_code',Auth::user()->business_code)->where('section_code',$this->sectionCode)->where('job',$code)->orderby('id','asc')->get();
      ...
      $sectionDetails =  sections::where('section_code',$this->sectionCode)->where('business_code',Auth::user()->business_code)->first();

      $sections = sections::where('job_code',$this->jobCode)->where('business_code',Auth::user()->business_code)->orderby('id','desc')->get();

      return view('livewire.jobs.job.task-section', compact('job','code','groups','members','sections','sectionDetails'));
   }
   ...
   public function add_task_group(){
      ...
      if($this->group_section){
         $secCode = $this->group_section;
      }else{
         $secCode = $this->sectionCode;
      }
   ...
   //edit section
   public function edit_section($code){
      $this->editSectionModal = "on";
      $editSection = sections::where('business_code',Auth::user()->business_code)->where('section_code',$code)->first();
      $this->section_title = $editSection->title;
      $this->SectionEditCode = $code;
   }

   //update section
   public function update_section(){
      $updateSections = sections::where('business_code',Auth::user()->business_code)->where('section_code',$this->SectionEditCode)->first();
      $updateSections->title      = $this->section_title;
      $updateSections->updated_by = Auth::user()->user_code;
      $updateSections->save();

      //record activity
      $activity     = '<a href="#">'.Auth::user()->name.'</a> has <b>updated</b> task section <a href="#">'.$this->section_title.'</a>';
      $module       = 'Jobs Management';
      $section      = 'Section';
      $action       = 'Edit';
      $activityCode = $this->SectionEditCode;
   ...
   //remove section
   public function remove_section($code){
   ...
   public function delete_section(){
      sections::where('business_code',Auth::user()->business_code)->where('section_code',$this->SectionEditCode)->delete();

      $getGroups = group::where('business_code',Auth::user()->business_code)
                        ->where('section_code',$this->SectionEditCode)
                        ->orderby('id','asc')
                        ->get();

      foreach($getGroups as $grp){
         //group
         $group = group::where('business_code',Auth::user()->business_code)->where('group_code',$grp->group_code)->first();

         //task
         $tasks = JobsTasks::where('group',$grp->group_code)->where('business_code',Auth::user()->business_code)->get();
         foreach($tasks as $task){
      ...
      return redirect()->route('job.task',$this->jobCode);
