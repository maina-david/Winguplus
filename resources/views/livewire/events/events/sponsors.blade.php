   ...
   @foreach($sponsors as $logo)
      <div class="col-md-3 mb-2">
         <a href="#" class="widget-card rounded mb-20px" data-id="widget">
            <div class="widget-card-cover rounded" style="background-image: url({!! asset('storage/account/businesses/'.Wingu::business()->business_code.'/events/'.$logo->file_name) !!});height:150px"></div>
            <div class="widget-card-content"></div>
            <div class="widget-card-content bottom">
               <h4 class="text-white mt-5"><b><br></b></h4>
            </div>
            <h4 class="mt-4">{!! $logo->name !!}</h4>
            <a href="" class="btn btn-sm btn-primary" wire:click="edit({{$logo->id}})"  data-toggle="modal" data-target="#sponsorModal"><i class="fas fa-edit"></i> Edit</a>
            <a href="" class="btn btn-sm btn-danger" wire:click="confirm_delete({{$logo->id}})"  data-toggle="modal" data-target="#delete"><i class="fas fa-trash-alt"></i> Delete</a>
         </a>
      </div>
   @endforeach
