            ...
            <div class="modal-body">
               @foreach($notes as $note)
                  ...
                  <div class="note-side js-single-note">
                     <a href="#" wire:click="delete_confirmation({{$getCode}})" data-toggle="modal" data-target="#deleteSideNote"><span class="note-remove js-note-remove"></span></a>
                     <a wire:click="view_note({{$getCode}})" data-toggle="modal" data-target="#rightSideNoteDetails">
                        <div class="note-text" wire:click="edit({{$getCode}})">
                           {!! nl2br($note->note) !!}
                        </div>
                        <div class="note-date">
                           {!! date('F jS,Y h:i:s A', strtotime($note->created_at)) !!}
                        </div>
                     </a>
                  </div>
               @endforeach
         ...
         <div class="modal-dialog" role="document">
            <div class="modal-content">
               ...
               <div class="modal-footer modal-footer-fixed">
                  ...
                  <button type="button" class="btn btn-primary" wire:click="edit({{$viewCode}})" data-toggle="modal" data-target="#leftSlideNoteArea"><i class="fa fa-edit"></i> Edit</button>
               </div>
            </div>
         </div>
      </div>
