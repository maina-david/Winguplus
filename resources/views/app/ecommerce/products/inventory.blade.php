@extends('layouts.app')
{{-- page header --}}
@section('title','Product Inventory | E-commerce')
         ...
         <div class="panel panel-default">
            <div class="panel-body">
               <div class="col-md-12 mt-3">
                  {!! Form::model($product, ['route' => ['ecommerce.inventory.settings.update',$productCode], 'class' => 'row', 'method'=>'post','enctype'=>'multipart/form-data']) !!}
                     @csrf
                     <div class="col-md-6">
                           ...
                           <table class="table table-striped">
                              ...
                              <tbody>
                                 @foreach($inventories as $inventory)
                                    <form action="{!! route('ecommerce.products.inventory.update',[$inventory->id,$productCode]) !!}" method="POST" >
                                       @csrf
                                       <tr>
                                          <td>
                                          ...
                                          <td>
                                             ...
                                             @if($inventory->default_inventory != 'Yes')
                                                @if(Hr::check_branch($inventory->branch_code) == 1)
                                                   <a href="{!! route('ecommerce.inventory.outlet.link.delete',[$productCode,$inventory->branch_code]) !!}" class="btn btn-sm btn-danger delete"><i class="fas fa-trash"></i></a>
                                                @endif
                                             @endif
                                          </td>
                                       </tr>
      ...
      <div class="modal-dialog" role="document">
         <div class="modal-content">
            ...
            <div class="modal-body">
               <form action="{!! route('ecommerce.inventory.outlet.link') !!}" method="post" enctype="multipart/form-data">
                  @csrf
                     ...
                     <select name="outlets[]" id="" class="form-control select2" multiple required style="width:100%">
                        @foreach($outlets as $outlet)
                           @if($mainBranch->branch_code != $outlet->branch_code)
                              <option value="{!! $outlet->branch_code !!}">{!! $outlet->name !!}</option>
                           @endif
                        @endforeach
                     </select>
