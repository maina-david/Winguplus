...
@section('content')
   ...
   <div id="content" class="content">
      ...
      <!-- begin page-header -->
      <h1 class="page-header"><i class="fal fa-exclamation-circle"></i> Leads by Status | {!!$statusRequest !!} | {!!  date('F j, Y', strtotime($start)) !!} to {!!  date('F j, Y', strtotime($end)) !!}</h1>
      @include('partials._messages')
      <div class="row">
         <div class="col-md-8">
            <form action="{!! route('crm.reports.leads.status.filter') !!}" method="GET" class="row" autocomplete="off">
               <div class="col-md-3">
                  <div class="form-group">
                     {!! Form::select('status',$status,null,['class'=>'form-control multiselect']) !!}
                  </div>
               </div>
               <div class="col-md-3">
                  <div class="form-group">
         ...
         <div class="col-md-4">
            <a href="{!! route('crm.reports.leads.status.export',[$statusValue,$start,$end]) !!}" class="float-right btn btn-pink"><i class="fal fa-file-excel"></i> Export to csv</a>
         </div>
         <div class="col-md-12">
            <div class="card">
               <div class="card-body">
                     ...
                     <tbody>
                        @foreach ($leads as $lead)
                           <tr>
                              ...
                              <td>
                                 @if($lead->sourceID != 0)
                                    @if(Crm::check_sources($lead->sourceID) != 0)
                                       {!! Crm::source($lead->sourceID)->name !!}
                                    @endif                                    
                                 @endif
                              </td>
                              <td>
