	...
	<div id="content" class="content">
		<div class="row">
			<div class="col-md-3">
               ...
               @if($client->image != "")
						<img src="{!! asset('businesses/'.Wingu::business(Auth::user()->business_code)->business_code.'/customer/'.$client->customer_code.'/images/'.$client->image) !!}" alt="" style="width:90px;height:90px" class="rounded-circle">
					@else
					...
					<a href="{{ route('finance.contact.edit', $customerCode) }}" class="btn btn-success btn-xs waves-effect mb-2 waves-light"><i class="fal fa-edit"></i> Edit</a>
					@if(Wingu::business()->plan != 1)
						<a href="{!! route('finance.customer.mail',$customerCode) !!}" class="btn btn-primary btn-xs waves-effect mb-2 waves-light"><i class="fal fa-paper-plane"></i> Send Email</a>
					@endif
					<a href="{!! route('finance.contact.delete', $customerCode) !!}" class="btn btn-danger btn-xs waves-effect mb-2 waves-light delete"><i class="fas fa-trash"></i> Delete</a>
				</div>
			...
			<div class="col-md-9">
				<div class="row mb-3">
                  ...
                  <div class="float-right">
							<a href="{{ route('finance.contact.edit', $customerCode) }}" class="btn btn-white"><i class="fas fa-edit"></i> Edit</a>
							@if(Wingu::business()->plan != 1)
								<a href="{{ route('finance.customer.send', $customerCode) }}" class="btn btn-white"><i class="fal fa-paper-plane"></i> Send Mail</a>
							@endif
   						<div class="btn-group">
   						...
   						<div class="btn-group">
								...
								<ul class="dropdown-menu">
									@if(Wingu::business()->plan != 1)
										<li><a href="{!! route('finance.customer.sms',$customerCode) !!}"><i class="fal fa-sms"></i> SMS</a></li>
										<li><a href="{!! route('finance.customer.mail',$customerCode) !!}"><i class="fal fa-paper-plane"></i> Mail</a></li>
									@endif
									<li class="divider"></li>
									<li><a href="{!! route('finance.customer.notes',$customerCode) !!}"><i class="fal fa-sticky-note"></i> Notes</a></li>
									<li><a href="{!! route('finance.customer.events',$customerCode) !!}"><i class="fal fa-calendar-alt"></i> Meeting</a></li>
									<li><a href="{!! route('finance.customer.calllog',$customerCode) !!}"><i class="fal fa-phone-office"></i> Call log</a></li>
									<li class="divider"></li>
									<li><a href="{!! route('finance.contact.delete', $customerCode) !!}" class="delete"><i class="fas fa-trash"></i> Delete</a></li>
								</ul>
							</div>
							@if(Wingu::business()->plan != 1)
								<a href="{!! route('finance.customer.documents',$customerCode) !!}" class="btn btn-white"><i class="fal fa-folder"></i> Documents</a>
							@endif
                  </div>
					</div>
				</div>
				...
				<!-- main client page -->
				@if(Request::is('finance/customer/'.$customerCode.'/show'))
					@include('app.finance.contacts.show')
				@endif

				<!-- comments -->
				@if(Request::is('finance/customer/'.$customerCode.'/comments'))
					@include('app.finance.contacts.comments')
				@endif

				<!-- invoice -->
				@if(Request::is('finance/customer/'.$customerCode.'/invoices'))
					@include('app.finance.contacts.invoices')
				@endif

            <!-- estimates -->
				@if(Request::is('finance/customer/'.$customerCode.'/quotes'))
					@include('app.finance.contacts.quotes')
				@endif

            <!-- creditnote -->
				@if(Request::is('finance/customer/'.$customerCode.'/creditnotes'))
					@include('app.finance.contacts.creditnotes')
				@endif

				<!-- projects -->
				@if(Request::is('finance/customer/'.$customerCode.'/projects'))
					@include('app.finance.contacts.projects')
				@endif

				<!-- statement -->
				@if(Request::is('finance/customer/'.$customerCode.'/statement'))
					@include('app.finance.contacts.statement')
				@endif

				<!-- statement mail -->
				@if(Request::is('finance/customer/'.$customerCode.'/statement/mail'))
					@include('app.finance.contacts.statementMail')
				@endif

				<!-- subscription -->
				@if(Request::is('finance/customer/'.$customerCode.'/subscriptions'))
					@include('app.finance.contacts.subscriptions')
				@endif

            <!-- contacts -->
				@if(Request::is('finance/customer/'.$customerCode.'/contacts'))
					@include('app.finance.contacts.contacts')
				@endif

				<!-- mails -->
				@if(Request::is('finance/customer/'.$customerCode.'/mail'))
					@include('app.finance.contacts.mail.index')
				@endif

				<!-- send -->
				@if(Request::is('finance/customer/'.$customerCode.'/send'))
					@include('app.finance.contacts.mail.mail')
				@endif

				<!-- documents -->
				@if(Request::is('finance/customer/'.$customerCode.'/documents'))
					@include('app.finance.contacts.documents')
				@endif

				<!-- sms -->
				@if(Request::is('finance/customer/'.$customerCode.'/sms'))
					@include('app.finance.contacts.sms')
				@endif

				<!-- notes -->
				@if(Request::is('finance/customer/'.$customerCode.'/notes'))
					@include('app.finance.contacts.notes')
				@endif

				<!-- events -->
				@if(Request::is('finance/customer/'.$customerCode.'/events'))
					@include('app.finance.contacts.events')
				@endif

				<!-- events -->
				@if(Request::is('finance/customer/'.$customerCode.'/calllogs'))
