@extends('layouts.app')
{{-- page header --}}
@section('title','Settings | Sales order Settings')
            ...
            <div class="card">
               <div class="card-header">
                  <ul class="nav nav-tabs card-header-tabs">
                     <li class="nav-item">
                        <a class="{{ Nav::isRoute('finance.settings.salesorders') }}" href="{!! route('finance.settings.salesorders') !!}"><i class="fas fa-sort-numeric-up"></i> Generated Numbers</a>
                     </li>
                     <li class="nav-item">
                        <a class="{{ Nav::isResource('defaults') }}" href="{!! route('finance.settings.salesorders.defaults',$settings->id) !!}">
                           <i class="fas fa-file-invoice-dollar"></i> Defaults
                        </a>
                     </li>
                     <li class="nav-item">
                        <a class="{{ Nav::isResource('tabs') }}" href="{!! route('finance.settings.salesorders.tabs',$settings->id) !!}">
               ...
               <div class="card-block">
                  <div class="p-0 m-0">
                     @if(Request::is('finance/settings/salesorders'))
                        <div class="">
                           {!! Form::model($settings, ['route' => ['finance.settings.salesorders.generated.update',$settings->id], 'method'=>'post',]) !!}
                              {!! csrf_field() !!}
                              ...
                              {{-- <p class="font-weight-bold"><i class="fas fa-info-circle text-primary"></i> Specify a prefix to dynamically set the Estimate number. The Estimate number will have 2 extra zeros i.e 002.</p> --}}
                              <div class="form-group">
                                 <center>
                     ...
                     @endif
                     @if(Request::is('finance/settings/salesorders/'.$settings->id.'/defaults'))
                        <div class="">
                           {!! Form::model($settings, ['route' => ['finance.settings.salesorders.defaults.update',$settings->id], 'method'=>'post',]) !!}
                              {!! csrf_field() !!}
                              <div class="form-group">
                     ...
                     @endif
                     @if(Request::is('finance/settings/salesorders/'.$settings->id.'/tabs'))
                        <div class="">
                           {!! Form::model($settings, ['route' => ['finance.settings.salesorders.tabs.update',$settings->id], 'method'=>'post',]) !!}
                              {!! csrf_field() !!}
