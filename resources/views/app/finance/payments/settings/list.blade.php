...
<table id="data-table-default" class="table table-striped table-bordered table-hover">
   ...
   <tbody>
      @foreach($methods as $count=>$method)
         <tr>
            <td>{!! $count+1 !!}</td>
            <td>{!! $method->name !!}</td>
            <td>{!! $method->description !!}</td>
            <td>
               <a href="#modal-dialog{!! $method->method_code !!}" class="btn btn-sm btn-primary" data-toggle="modal"><i class="fas fa-edit"></i> Edit</a>
               <a href="{!! route('finance.payment.mode.delete', $method->method_code) !!}" class="btn btn-sm btn-danger delete"><i class="fas fa-trash"></i> Delete</a>
            </td>
         </tr>
         <!-- #modal-dialog -->
         <div class="modal fade" id="modal-dialog{!! $method->method_code !!}">
            <div class="modal-dialog">
               {!! Form::model($method, ['route' => ['finance.payment.mode.update',$method->method_code], 'method'=>'post',]) !!}
