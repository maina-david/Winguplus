...
{{-- page header --}}
@section('title')
	Purchase Order | {!! $lpo->lpo_prefix !!}{!! $lpo->lpo_number !!}
@endsection
...
@section('content')
	<div id="content" class="content">
		...
		<div class="row">
			<div class="col-md-6">
				<a href="#" class="btn {!! $lpo->status_name !!} mb-2">
               <i class="fas fa-bell"></i> {!! $lpo->status_name !!}
            </a>
			</div>
			<div class="col-md-6">
				<div class="row">
					...
					<div class="col-md-8">
						<a href="{!! route('finance.stock.mail', $lpo->lpoCode) !!}" class="btn btn-sm btn-default m-b-10 p-l-5">
							<i class="fas fa-envelope"></i> Email
						</a>
						<a href="{!! route('finance.product.stock.order.edit', $lpo->lpoCode) !!}" class="btn btn-sm btn-default m-b-10 p-l-5">
							<i class="fas fa-edit"></i> Edit
						</a>
						<a href="{!! route('finance.product.stock.order.convert', [$lpo->lpoCode,'pdf']) !!}" target="_blank" class="btn btn-sm btn-white m-b-10 p-l-5">
							<i class="fa fa-file-pdf t-plus-1 text-danger fa-fw fa-lg"></i> Export as PDF
						</a>
						<a href="{!! route('finance.product.stock.order.convert', [$lpo->lpoCode,'print']) !!}" target="_blank" class="btn btn-sm btn-white m-b-10 p-l-5">
						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu dropdown-menu-right">
								...
								@if($lpo->status != 10)
									<li><a href="{!! route('finance.lpo.status.change',[$lpo->lpoCode,10]) !!}">Mark as Draft</a></li>
								@endif
								@if($lpo->status != 11)
								<li><a href="{!! route('finance.lpo.status.change',[$lpo->lpoCode,11]) !!}">Mark as Expired</a></li>
								@endif
								@if($lpo->status != 12)
								<li><a href="{!! route('finance.lpo.status.change',[$lpo->lpoCode,12]) !!}">Mark as Declined</a></li>
								@endif
								@if($lpo->status != 13)
								<li><a href="{!! route('finance.lpo.status.change',[$lpo->lpoCode,13]) !!}">Mark as Accepted</a></li>
								@endif
								@if($lpo->status != 6)
								<li><a href="{!! route('finance.lpo.status.change',[$lpo->lpoCode,6]) !!}">Mark as Sent</a></li>
								@endif
								@if($lpo->status != 14)
								<li><a href="{!! route('finance.stock.delivered',[$lpo->lpoCode]) !!}">Mark as Delivered</a></li>
								@endif
								<li class="divider"></li>
								@permission('delete-stockcontrol')
								<li><a href="{!! route('finance.lpo.delete',$lpo->lpoCode) !!}" class="text-danger">Delete LPO</a></li>
								@endpermission
							</ul>
						</div>
					</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="{!! route('finance.stock.attach') !!}" class="dropzone" id="my-awesome-dropzone" method="post">
						@csrf()
						<input type="hidden" value="{!! $lpo->lpoCode !!}" name="lpoCode">
					</form>
